...
/**
 * Links a certain tab or tab-switch to their respective arrays, creating an unique id and the neccessary events.
 * @param array - either the tabs or switches array
 */
export function useTabLink(array: Ref<Array<number | string>> | undefined) {
  // Create a random id
  const id: Ref<string | number | null> = ref(randomString());
